...
from mplkorfball._pitch_base import BasePitch
from mplkorfball.heatmap import bin_statistic, heatmap
from mplkorfball.linecollection import lines
from mplkorfball.quiver import arrows
from mplkorfball.scatterutils import scatter_korfball, scatter_rotation
from mplkorfball.utils import validate_ax, copy_doc
...
class BasePitchPlot(BasePitch):
    ...
    def scatter(self, x, y, rotation_degrees=None, marker=None, ax=None, **kwargs):
        ...
        which automatically flips the x and y coordinates if the pitch is vertical.
        You can optionally use a korfball marker with marker='korfball' and rotate markers with
        rotation_degrees.
        ...
        marker: MarkerStyle, optional
            ...
            If marker='korfball' plots a korfball shape with the pentagons the color
            of the edgecolors and hexagons the color of the 'c' argument; 'linewidths'
            also sets the linewidth of the korfball marker.
        ax : matplotlib.axes.Axes, default None
            The axis to plot on.
        **kwargs : All other keyword arguments are passed on to matplotlib.axes.Axes.scatter.
        ...
        paths : matplotlib.collections.PathCollection
                or a tuple of (paths, paths) if marker='korfball'
        ...
        >>> from mplkorfball import Pitch
        >>> from mplkorfball import arrowhead_marker
        >>> pitch = Pitch()
        >>> fig, ax = pitch.draw()
        ...
        >>> pitch = Pitch()
        >>> fig, ax = pitch.draw()
        >>> pitch.scatter(30, 30, marker='korfball', ax=ax)
        """
        validate_ax(ax)
        x = np.ma.ravel(x)
        y = np.ma.ravel(y)
        ...
        if marker == 'korfball' and rotation_degrees is not None:
            raise NotImplementedError("rotated korfball markers are not implemented.")

        if marker == 'korfball':
            return scatter_korfball(x, y, ax=ax, **kwargs)
    ...
    def korf_angle(self, x, y, ax=None, korf='right', **kwargs):
        ...
        **kwargs : All other keyword arguments are passed on to
        ...
        >>> pitch = Pitch()
        >>> fig, ax = pitch.draw()
        >>> pitch.korf_angle(100, 30, alpha=0.5, color='red', ax=ax)
        """
        validate_ax(ax)
        valid_korf = ['left', 'right']
        if korf not in valid_korf:
            raise TypeError(f'Invalid argument: goal should be in {valid_korf}')
        x = np.ravel(x)
        y = np.ravel(y)
        if x.size != y.size:
            raise ValueError("x and y must be the same size")
        korf_coordinates = self.korf_right if korf == 'right' else self.korf_left
        verts = np.zeros((x.size, 3, 2))
        verts[:, 0, 0] = x
        verts[:, 0, 1] = y
        verts[:, 1:, :] = np.expand_dims(korf_coordinates, 0)
