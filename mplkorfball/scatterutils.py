...
# korfball hexagon arrays
korfball_hexagon_codes = np.array([1, 3, 3, 3, 3, 4, 4, 4, 3, 3, 3, 3, 3, 3, 79, 1, 3,
                                   3, 3, 3, 4, 4, 4, 3, 3, 3, 3, 3, 3, 79, 1, 3, 3, 3,
                                   3, 4, 4, 4, 3, 3, 2, 3, 3, 79, 1, 2, 3, 3, 4, 4, 4,
                                   3, 3, 3, 3, 3, 3, 79, 1, 3, 3, 3, 3, 4, 4, 4, 3, 3,
                                   3, 3, 3, 3, 79, 1, 1, 1, 1])
korfball_hexagon_vertices = np.array([[-0.22499999999999998, -0.3096859321060139],
...
# korfball pentagon arrays
korfball_pentagon_codes = np.array([1, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 79, 1, 3, 3, 4, 4,
                                    4, 3, 3, 79, 1, 3, 3, 4, 4, 4, 3, 3, 79, 1, 3, 3, 4,
                                    4, 4, 3, 3, 79, 1, 3, 3, 4, 4, 4, 3, 3, 79, 1, 3, 3,
                                    4, 4, 4, 3, 3, 79, 1, 1, 1, 1])
korfball_pentagon_vertices = np.array([[0.000000000000000023439302766909766, 0.38279286375841787],
                                       [-0.21202882373436308, 0.28054118199261147],
...
korfball_hexagon_marker = mpath.Path(korfball_hexagon_vertices, korfball_hexagon_codes)
korfball_pentagon_marker = mpath.Path(korfball_pentagon_vertices, korfball_pentagon_codes)
...
def scatter_korfball(x, y, ax=None, ballstyle='dutch', **kwargs):
    """ Scatter plot of korfball markers.
    Plots two scatter plots one for the hexagons and one for the pentagons of the korfball.
    ...
    **kwargs : All other keyword arguments are passed on to matplotlib.axes.Axes.scatter.
    ...
    if ballstyle == 'dutch':
        ...
        pentcolor = kwargs.pop('edgecolors', 'orange')
    elif ballstyle == 'new':
        hexcolor = kwargs.pop('c', 'yellow')
        pentcolor = kwargs.pop('edgecolors', 'blue')
    else:
        hexcolor = kwargs.pop('c', 'white')
    ...
    sc_hex = ax.scatter(x, y, edgecolors=pentcolor, c=hexcolor, linewidths=linewidths,
                        marker=korfball_hexagon_marker, s=s, **kwargs)

    if 'label' in kwargs:
        Legend.update_default_handler_map({sc_hex: HandlerKorfball()})
        del kwargs['label']

    sc_pent = ax.scatter(x, y, edgecolors=pentcolor, c=pentcolor, linewidths=linewidths,
                         marker=korfball_pentagon_marker, s=s, **kwargs)

    return sc_hex, sc_pent


class HandlerKorfball(HandlerPathCollection):
    """Automatically generated by scatter_korfball() if label is a keyword
    to allow use of korfball marker in legend."""
    ...
    if 'transOffset' in inspect.signature(HandlerPathCollection.create_collection).parameters.keys():
        def create_collection(self, orig_handle, sizes, offsets, transOffset):
            ...
            return type(orig_handle)([korfball_hexagon_marker, korfball_pentagon_marker],
                                     sizes=sizes,
                                     offsets=offsets,
